...
<template>
  <div
    class="absolute top-2 right-2 flex w-full max-w-xs items-center rounded-lg bg-white p-4 text-gray-500 shadow dark:bg-gray-800 dark:text-gray-400"
    role="alert"
  >
    <div
      v-if="!TOAST_STATE.toast.isSuccess"
    ...
    <div class="ml-3 text-sm font-normal">{{ TOAST_STATE.toast.message }}</div>
    <button
      type="button"
      ...
      aria-label="Close"
      @click="TOAST_STATE.toast.status = false"
    >
